...
local function greetCallback(cid)
	...
	if player then
		npcHandler:setMessage(MESSAGE_GREET, "Greetings. A warning straight ahead: I don't like loiterin'. If you're not here to {help} us, you're here to waste my time. Which I consider loiterin'. Now, try and prove your {worth} to our alliance. ... ",
											  "I have sealed some of the areas far too dangerous for anyone to enter. If you can prove you're capable, you'll get an opportunity to help destroy the weird machines, pumping lava into the caves leading to the most dangerous enemies.", cid)
		playerTopic[cid] = 1
	end
...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "subterraneans") and npcHandler.topic[cid] == 1 then
		if player:getStorageValue(Storage.DangerousDepths.Dwarves.Subterranean) == 2 and player:getStorageValue(Storage.DangerousDepths.Dwarves.timeTaskSubterranean) > 0 then
			...
			local expirationTime = player:getStorageValue(Storage.DangerousDepths.Dwarves.timeTaskSubterranean)
			if expirationTime > currentTime then
				...
				npcHandler:say("I don't need your help for now. Come back later. It will expire in {" .. timeString .. "}.", cid)
				playerTopic[cid] = 1
			end
		end
		if player:getStorageValue(Storage.DangerousDepths.Dwarves.Subterranean) == 2 and player:getStorageValue(Storage.DangerousDepths.Dwarves.timeTaskSubterranean) <= 0 then
	...
	if msgcontains(msg, "home") and npcHandler.topic[cid] == 1 then
		if player:getStorageValue(Storage.DangerousDepths.Dwarves.Home) == 2 and player:getStorageValue(Storage.DangerousDepths.Dwarves.timeTaskHome) > 0 then
			local currentTime = os.time()
			local expirationTime = player:getStorageValue(Storage.DangerousDepths.Dwarves.timeTaskHome)
			if expirationTime > currentTime then
				local remainingTime = expirationTime - currentTime
				local timeString = getTimeRemainingText(remainingTime)
	...
	elseif msgcontains(msg, "gnomes") and npcHandler.topic[cid] == 56 then
		if player:getItemCount(27653) >= quantidade[cid] then
			...
			player:removeItem(27653, quantidade[cid])
