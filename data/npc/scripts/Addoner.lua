...
local outfits = {'citizen', 'hunter', 'knight', 'mage', 'nobleman', 'summoner', 'warrior', 'barbarian', 'druid', 'wizard', 'oriental', 'pirate', 'assassin', 'beggar', 'shaman', 'norseman', 'nightmare', 'jester', 'yalaharian', 'brotherhood'}
...
function creatureSayCallback(cid, type, msg)
    ...
    elseif msgcontains(msg, "yes") then
        if (talkState[talkUser] > 10010 and talkState[talkUser] < 10100) then
            ...
            if (getPlayerMoney(cid) >= addoninfo[rtnt[talkUser]].cost) and (items_number == table.maxn(addoninfo[rtnt[talkUser]].items)) then
                ...
                npcHandler:say('You do not have the needed items!', cid)
            end
            rtnt[talkUser] = nil
    ...
    elseif msgcontains(msg, "addon") then
        npcHandler:say('I can give you addons for {' .. table.concat(outfits, "}, {") .. '} outfits.', cid)
        rtnt[talkUser] = nil
        talkState[talkUser] = 0
        npcHandler:resetNpc()
        return true
