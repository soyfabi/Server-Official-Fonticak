...
local function greetCallback(cid)
	...
	if player:getStorageValue(Storage.Kilmaresh.First.Access) < 1 then
		npcHandler:setMessage(MESSAGE_GREET, "Greetings, seeker of knowledge. You seem to be just the person I'm {looking} for.") -- It needs to be revised, it's not the same as the global
...
local function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "search") then
		...
		npcHandler:say({
			...
			"My leads are the {museum} in Thais, something strange in the Darashian {desert}, rumours about {fishmen}, an ancient order, the mysterious {asuri}, or a lost {isle}?"
		}, cid)
		npcHandler.topic[cid] = 1
	elseif msgcontains(msg, "museum") and npcHandler.topic[cid] == 1 then
		npcHandler:say({
			"I have heard that it was recently planned to expand the Museum of Tibian Arts. In the course of these activities unexpected difficulties occured."
		}, cid)
...
keywordHandler:addKeyword({'fishmen'}, StdModule.say, {npcHandler = npcHandler, text = "I'm sorry. You seem to not own the required information yet."})
