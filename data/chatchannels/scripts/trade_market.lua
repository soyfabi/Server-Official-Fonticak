...
function onSpeak(player, type, message)
	...
	if player:getCondition(CONDITION_CHANNELMUTEDTICKS, CONDITIONID_DEFAULT, CHANNEL_MARKET) then
		player:sendCancelMessage("You are muted from the Trade Market channel for using it inappropriately.")
		return false
	end
			...
			if target then
				if playerAccountType > target:getAccountType() then
					if not target:getCondition(CONDITION_CHANNELMUTEDTICKS, CONDITIONID_DEFAULT, CHANNEL_MARKET) then
						...
						sendChannelMessage(CHANNEL_MARKET, TALKTYPE_CHANNEL_R1, target:getName() .. " has been muted by " .. player:getName() .. " for using Trade Market Channel inappropriately.")
					else
