...
function distortedSource.onThink(creature)
	...
	elseif creature:getHealth() >= 55000 or getGlobalStorageValue(GlobalStorage.ForgottenKnowledge.AstralGlyph) < 1 then
	...
	if getGlobalStorageValue(GlobalStorage.ForgottenKnowledge.AstralGlyph) >= 1 then
		local spectators = Game.getSpectators(Position(31986, 32847, 14), false, false, 12, 12, 12, 12)
		for i = 1, #spectators do
