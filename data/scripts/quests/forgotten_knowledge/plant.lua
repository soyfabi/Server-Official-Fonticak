...
local function revertTree()
	...
	setGlobalStorageValue(GlobalStorage.ForgottenKnowledge.ActiveTree, 0)
	for v = 1, #dry do
...
function forgottenKnowledgePlant.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	elseif item.itemid == 23810 then
		...
		elseif target.itemid == 6180 and target:getPosition() == Position(32737, 32116, 10) then
			if getGlobalStorageValue(GlobalStorage.ForgottenKnowledge.ActiveTree) >= 1 then
			...
			setGlobalStorageValue(GlobalStorage.ForgottenKnowledge.ActiveTree, 1)
