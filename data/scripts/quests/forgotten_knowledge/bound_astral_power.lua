...
function astralPower.onKill(creature, target)
	...
	setGlobalStorageValue(GlobalStorage.ForgottenKnowledge.AstralPowerCounter,
	getGlobalStorageValue(GlobalStorage.ForgottenKnowledge.AstralPowerCounter) + 1)
	if getGlobalStorageValue(GlobalStorage.ForgottenKnowledge.AstralPowerCounter) >= 4 then
		setGlobalStorageValue(GlobalStorage.ForgottenKnowledge.AstralPowerCounter, 1)
	end
	...
	for i = 1, #positions do
		if creature:getPosition():getDistance(positions[i].pos) < 7 then
			...
			setGlobalStorageValue(GlobalStorage.ForgottenKnowledge.AstralGlyph, 1)
			addEvent(setGlobalStorageValue, 1 * 60 * 1000, GlobalStorage.ForgottenKnowledge.AstralGlyph, 0)
		end
	end
	return true
end
