...
local Crystals = {
    {x = 32776, y = 31804, z = 10}, -- Dark Blue Crystal
...
function bigfootMusic.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    if player:getStorageValue(Storage.BigfootBurden.QuestLine) == 21 then
        ...
        if itemPosition.x == 32728 and itemPosition.y == 32878 and itemPosition.z == 7 then
            -- Realizar acciones si la posición del item coincide con (32728, 32878, 7)
            -- Tu lógica aquí
        else
            for i, crystalPos in ipairs(Crystals) do
                if itemPosition.x == crystalPos.x and itemPosition.y == crystalPos.y and itemPosition.z == crystalPos.z then
