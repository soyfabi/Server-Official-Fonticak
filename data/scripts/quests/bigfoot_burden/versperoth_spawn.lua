...
local function executeVersperothBattle(mid)
	if getGlobalStorageValue(GlobalStorage.BigfootBurden.Versperoth.Battle) ~= 1 then
		return false
	end
	...
	if mid then
		...
		setGlobalStorageValue(GlobalStorage.BigfootBurden.Versperoth.Health, monster:getHealth())
		monster:remove()
	...
	if monster then
		...
		monster:setHealth(getGlobalStorageValue(GlobalStorage.BigfootBurden.Versperoth.Health))
...
function versperothSpawn.onStepIn(creature, item, position, fromPosition)
	...
	setGlobalStorageValue(GlobalStorage.BigfootBurden.Versperoth.Battle, 1)
	setGlobalStorageValue(GlobalStorage.BigfootBurden.Versperoth.Health, 100000)
