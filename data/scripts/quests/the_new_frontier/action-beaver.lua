...
function beaverTrees.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if toPosition == config.treesBeaver[1] or toPosition == config.treesBeaver[2] or toPosition == config.treesBeaver[3] and player:getStorageValue(TheNewFrontier.Questline) == 5 then
		...
		elseif toPosition == config.treesBeaver[2] and player:getStorageValue(TheNewFrontier.Mission02[1]) == 3 and player:getStorageValue(TheNewFrontier.Mission02.Beaver2) < 1 then
			for i = 1, 5 do
				pos = toPosition
			...
			player:setStorageValue(TheNewFrontier.Mission02[1], 4)
		...
		elseif toPosition == config.treesBeaver[3] and player:getStorageValue(TheNewFrontier.Mission02[1]) == 4 and player:getStorageValue(TheNewFrontier.Mission02.Beaver3) < 1 then
			Game.createMonster("thieving squirrel", toPosition)
			toPosition:sendMagicEffect(CONST_ME_TELEPORT)
			player:setStorageValue(TheNewFrontier.Mission02.Beaver3, 1)
			player:setStorageValue(TheNewFrontier.Mission02[1], 5)
