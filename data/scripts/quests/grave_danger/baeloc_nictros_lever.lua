...
function baelocNictrosLever.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if lever:checkConditions() then
		...
		addEvent(function()
			...
			player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You were kicked because time is up, come back in 20 hours.")
			spec:removePlayers(player_remove)
		end, config.timeToDefeatBoss * 60 * 1000)
	end
end

baelocNictrosLever:uid(61000)
