...
local function createArmor(id, amount, pos)
	...
	if armor then
		...
		armor:getPosition():sendMagicEffect(CONST_ME_THUNDER)
	end
end
	...
	if item.actionid == 40003 then
		...
		elseif item.itemid == entry.metalWallId then
			if player:getPosition().y == entry.roomExitPos.y then
				local timeremaining = player:getStorageValue(info.timer) - os.time()
				...
				if timeremaining > 0 then
					...
					local message = "You are still exhausted from your last battle, wait"
