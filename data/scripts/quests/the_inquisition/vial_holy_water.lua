...
function othersHolyWater.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	elseif target.actionid == 2003 then
		...
		local posdoor = Position(32260, 32791, 7)
		if doorItem then
			doorItem:transform(7868)
			posdoor:sendMagicEffect(CONST_ME_POFF)
			player:say('Thud!', TALKTYPE_MONSTER_SAY, false, player, Position(32260, 32791, 7))
		end
	...
	if isInArray({7925, 7927, 7929}, target.itemid) then
		if target.itemid == 7929 then
			setGlobalStorageValue(GlobalStorage.Inquisition, math.random(4,5))
		end
	...
	elseif target.itemid == 7931 then
		if getGlobalStorageValue(GlobalStorage.Inquisition) > 0 then
			setGlobalStorageValue(GlobalStorage.Inquisition, (Game.getStorageValue(GlobalStorage.Inquisition)-1))
