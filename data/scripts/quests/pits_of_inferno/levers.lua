...
local function createStones()
	...
	setGlobalStorageValue(GlobalStorage.PitsOfInfernoLevers, 0)
end
...
function pitsOfInfernoLevers.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	local leverCount = math.max(0, getGlobalStorageValue(GlobalStorage.PitsOfInfernoLevers))
	if item.uid > 2049 and item.uid < 2065 then
		...
		setGlobalStorageValue(GlobalStorage.PitsOfInfernoLevers, number)
