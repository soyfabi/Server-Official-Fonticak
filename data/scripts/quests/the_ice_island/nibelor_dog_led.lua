...
function nibelorDogLed.onStepIn(creature, item, position, fromPosition)
	...
	for b = 1, #setting do
		if player:getPosition() == setting[b].sledPosition then
			if player:getStorageValue(setting[b].storage) == setting[b].value then
				if player:removeItem(3582, 1) then
					player:teleportTo(setting[b].destination)
					setting[b].destination:sendMagicEffect(CONST_ME_TELEPORT)
				else
					player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You need Ham for the dogs.")
					player:teleportTo(fromPosition)
				end
			else
				player:teleportTo(fromPosition)
				player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You need complete the missions of The Ice Island.")
			end
		end
	end
	return true
