        ...
        if (#rPlayers >= bossConfig.minPlayersRequired) then
            for _, pid in pairs(rPlayers) do
                ...
                if rplayer:isPlayer() then
					if rplayer:getStorageValue(Storage.TheSecretLibrary.LiquidDeath) == 11 then
						...
						rplayer:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Someone does not have complete access.")
					end
                end
            end
