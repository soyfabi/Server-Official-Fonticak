local config = {
	...
	{ position = { x = 33584, y = 31388, z = 13 }, destination = { x = 33584, y = 31391, z = 13 } },
}
...
function moveEvent.onStepIn(creature, item, position, fromPosition)
	...
	for value in pairs(config) do
		if Position(config[value].position) == player:getPosition() then
			...
			player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You squeeze through an ancient small passage. There are small symbols carved deep into the coral you cannot quite read.")
			return true
		end
	end
end
