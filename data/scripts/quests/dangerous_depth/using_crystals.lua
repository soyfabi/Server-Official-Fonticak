...
function dangerousDepthCrystals.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if targetPosition == WarzoneIV and target:getId() == geodeId then -- Warzone 4 BOSS!!!
		if getGlobalStorageValue(GlobalStorage.DangerousDepths.Geodes.WarzoneIV) < 30 then
			targetPosition:sendMagicEffect(CONST_ME_HITAREA)
			item:remove(1)
			if getGlobalStorageValue(GlobalStorage.DangerousDepths.Geodes.WarzoneIV) < 0 then
				setGlobalStorageValue(GlobalStorage.DangerousDepths.Geodes.WarzoneIV, 0)
			end
			setGlobalStorageValue(GlobalStorage.DangerousDepths.Geodes.WarzoneIV, getGlobalStorageValue(GlobalStorage.DangerousDepths.Geodes.WarzoneIV) + 1)
			if getGlobalStorageValue(GlobalStorage.DangerousDepths.Geodes.WarzoneIV) == 30 then
	...
	if targetPosition == WarzoneV and target:getId() == geodeId then -- Warzone 5 BOSS!!!
		if getGlobalStorageValue(GlobalStorage.DangerousDepths.Geodes.WarzoneV) < 30 then
			targetPosition:sendMagicEffect(CONST_ME_HITAREA)
			item:remove(1)
			if getGlobalStorageValue(GlobalStorage.DangerousDepths.Geodes.WarzoneV) < 0 then
				setGlobalStorageValue(GlobalStorage.DangerousDepths.Geodes.WarzoneV, 0)
			end
			setGlobalStorageValue(GlobalStorage.DangerousDepths.Geodes.WarzoneV, getGlobalStorageValue(GlobalStorage.DangerousDepths.Geodes.WarzoneV) + 1)
			if getGlobalStorageValue(GlobalStorage.DangerousDepths.Geodes.WarzoneV) == 30 then
				local spectators = Game.getSpectators(targetPosition, false, true, 3, 3, 3, 3)
				for _, spectator in pairs(spectators) do
					if spectator:isPlayer() then
	...
	if targetPosition == WarzoneVI and target:getId() == geodeId then -- Warzone 6 BOSS!!!
		if getGlobalStorageValue(GlobalStorage.DangerousDepths.Geodes.WarzoneVI) < 30 then
			targetPosition:sendMagicEffect(CONST_ME_HITAREA)
			item:remove(1)
			if getGlobalStorageValue(GlobalStorage.DangerousDepths.Geodes.WarzoneVI) < 0 then
				setGlobalStorageValue(GlobalStorage.DangerousDepths.Geodes.WarzoneVI, 0)
			end
			setGlobalStorageValue(GlobalStorage.DangerousDepths.Geodes.WarzoneVI, getGlobalStorageValue(GlobalStorage.DangerousDepths.Geodes.WarzoneVI) + 1)
			if getGlobalStorageValue(GlobalStorage.DangerousDepths.Geodes.WarzoneVI) == 30 then
				local spectators = Game.getSpectators(targetPosition, false, true, 3, 3, 3, 3)
				for _, spectator in pairs(spectators) do
					if spectator:isPlayer() then
