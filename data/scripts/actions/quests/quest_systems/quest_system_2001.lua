...
function questSystem1.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if size == 0 then
		...
		if not reward then
			print("[questSystem1.onUse] failed to create reward item")
			return false
		end
		...
		for i = 0, container:getSize() - 1 do
			...
			local newItem = Game.createItem(originalItem.itemid, originalItem.type)
			if not newItem then
				print("[questSystem1.onUse] failed to create new item")
				return false
			end
	...
	if reward then
		...
		for i = 1, size do
			...
			if reward:addItemEx(tmp) ~= RETURNVALUE_NOERROR then
				print("[questSystem1.onUse] - Could not add quest reward to container")
			end
		end
		local ret = ItemType(reward.itemid)
		result = ret:getArticle() .. ' ' .. ret:getName()
