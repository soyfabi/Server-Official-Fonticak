...
function frags.onSay(player, words, param)
    local fragTime = configManager.getNumber(configKeys.RED_SKULL_LENGTH)
    if fragTime <= 0 then
        player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "You do not have any unjustified kills.")
        return false
    end
